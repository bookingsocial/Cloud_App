...
class Auth extends MX_Controller
{
	...
	function index()
	{
		if ($message = $this->session->flashdata('message')) {
			...
			$page['page_name'] = 'auth/general_message';
			$page['data'] = array('message' => $message);
			$this->load->view('user_template',$page );
		} else {
			redirect('/bksl/auth/login/');
		}
	}
	/**
	...
	function is_logged_in()
	{
		if (!$this->tank_auth->is_logged_in()) {
			...
			//echo $hasher->HashPassword('booking123');
        	echo 'You don\'t have permission to access this page. <a href="bksl/auth/login">Login</a>';    
			die();
		}
	}
	/**
	...
	{
		return $this->tank_auth->get_user_by_id();
	}
	/**
	 * Login user on the site
	 *
	...
	function register()
	{
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			$page['showHeader'] = FALSE;
			$page['showFooter'] = FALSE;
			$page['page_name'] = 'auth/register_form';
			$page['data'] = $data;
			$this->load->view('user_template', $page);
		}
	}
	...
	function send_again()
	{
		if (!$this->tank_auth->is_logged_in(FALSE)) {							// not logged in or activated
			...
			$page['showHeader'] = FALSE;
			$page['showFooter'] = FALSE;
			$page['page_name'] = 'auth/send_again_form';
			$page['data'] = $data;
			$this->load->view('user_template', $page);
		}
	}
	...
	function activate()
	{
		...
		if ($this->tank_auth->activate_user($user_id, $new_email_key)) {		// success
			$this->tank_auth->logout();
			$this->_show_message($this->lang->line('auth_message_activation_completed').' '.anchor('/bksl/auth/login/', 'Login'));
	 ...
	 * @return void
	 */
	function a_deactivate()
	{
		$user_id		= $this->uri->segment(4);
		if ($this->tank_auth->is_logged_in()) {									// logged in
			
			// Deactivate user
			$this->tank_auth->admin_deactivate_user($user_id);
			redirect('bksl/viewuser/'.$user_id);
		}else{
			redirect('bksl/auth/login');
		}		
	}
	/**
	 * a_activate user account.
	 * User is verified by user_id and authentication code in the URL.
	 * Can be called by clicking on link in mail.
	 *
	 * @return void
	...
	function a_activate()
	{
		...
		if ($this->tank_auth->is_logged_in()) {									// logged in
			// Activate user
			$this->tank_auth->admin_activate_user($user_id);
			redirect('bksl/viewuser/'.$user_id);
		}else{
			redirect('bksl/auth/login');
		}		
	...
	function forgot_password()
	{
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			$page['showHeader'] = FALSE;
			$page['showFooter'] = FALSE;
			$page['page_name'] = 'auth/forgot_password_form';
			$page['data'] = $data;
			$this->load->view('user_template', $page);
		}
	}
	...
	function reset_password()
	{
		...
		if ($this->form_validation->run()) {								// validation ok
			if (!is_null($data = $this->tank_auth->reset_password(
				...
				$this->_show_message($this->lang->line('auth_message_new_password_activated').' '.anchor('/bksl/auth/login/', 'Login'));
		...
		$page['showHeader'] = FALSE;
		$page['showFooter'] = FALSE;
		$page['page_name'] = 'auth/reset_password_form';
		$page['data'] = $data;
		$this->load->view('user_template', $page);
	}
	...
	function change_password()
	{
		if (!$this->tank_auth->is_logged_in()) {								// not logged in or not activated
			...
			if($userDetails->user_type =='CONTACT'){
				$data['ContactDetails'] = $this->user_model->getContactDetails($related_to,$organizationId);
				...
				$page['page_name'] = 'auth/change_password_form';
	...
	function change_email()
	{
		if (!$this->tank_auth->is_logged_in()) {								// not logged in or not activated
			...
			$page['showHeader'] = FALSE;
			$page['showFooter'] = FALSE;
			$page['page_name'] = 'auth/change_email_form';
			$page['data'] = $data;
			$this->load->view('user_template', $page);
		}
	}
	...
	function reset_email()
	{
		...
		if ($this->tank_auth->activate_new_email($user_id, $new_email_key)) {	// success
			$this->tank_auth->logout();
			$this->_show_message($this->lang->line('auth_message_new_email_activated').' '.anchor('/bksl/auth/login/', 'Login'));
	...
	function unregister()
	{
		if (!$this->tank_auth->is_logged_in()) {								// not logged in or not activated
			...
			$page['showHeader'] = FALSE;
			$page['showFooter'] = FALSE;
			$page['page_name'] = 'auth/unregister_form';
			$page['data'] = $data;
			$this->load->view('user_template', $page);
		}
	}
