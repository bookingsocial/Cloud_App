...
class User_model extends CI_Model {
	...
	function createContact($inputs){
		$data =array(
				'Name'=>  $inputs['Last_Name__c'].' '.$inputs['First_Name__c'],
  				'FirstName' => $inputs['First_Name__c'],
  				'LastName' =>$inputs['Last_Name__c'],
  				'Email ' => $inputs['Email__c'],
  				'Mobile' => $inputs['Phone__c'],
	   ...
	   if($inputs['mode'] == 'e'){
		   $this->db->where('Salesforce_Id', $inputs['sfId']);
		   $response = $this->db->update('contacts', $data);
	    } else if($inputs['mode'] == 'c'){ 
		   $response = $this->db->insert('contacts', $data);
		   $insert_id = $this->db->insert_id();
		   $cont_date=date('dmY');
		   $datas =array(
				'Salesforce_Id' => '001'.$cont_date.$insert_id,
				'FirstName' => $inputs['First_Name__c']
			);
			$this->db->where('Id', $insert_id);
			$response = $this->db->update('contacts', $datas);
	    }
	   return $response;
	}
}
