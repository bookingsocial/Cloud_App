...
function AgendaEventRenderer() {
	...
	function createStatusNode( ){
		...
		for (var key in $.modalStatus) {
		  if ($.modalStatus.hasOwnProperty(key)) {
				   if($.modalStatus[key].hasOwnProperty('vc')){
								   
				   }else{
						statusHtml +='<div class="col-md-2" style="margin-right: 15px;">';
						statusHtml += '<div class="info-box">';
						statusHtml +='<span class="info-box-icon " style="background:'+$.modalStatus[key].color+';"></span>';
						statusHtml +='<div class="info-box-content"><span class="info-box-text">'+key+'</span><span class="info-box-number">'+$.modalStatus[key].count+'</span></div><!-- /.info-box-content -->';
						statusHtml +='</div><!-- /.info-box -->	</div><!-- /.col -->';
				   }
		  }
		}
		statusHtml +='';
	...
	function renderSlotSegs(segs, modifiedEventId) {
		...
		var i, segCnt=segs.length, seg,
			...
			isRTL = opt('isRTL');
		//var pendingCount = 0,canceledCount = 0,completedCount = 0,noshowcount = 0,approvedCount = 0;
		for (var key in $.modalStatus) {
		...
		for (i=0; i<segCnt; i++) {
			...
			if($.modalStatus.hasOwnProperty(event.status)){
				$.modalStatus[event.status].count++;
			}
		...
		$('#noshowCount').html('('+noshowcount+')');
		$('#canceledCount').html('('+canceledCount+')');
		$('#approvedCount').html('('+approvedCount+')');
		$('#pendingCount').html('('+pendingCount+')');*/
		$('#colorContent').html(createStatusNode);
